...
public class HalcyonEasyBehaviour : MonoBehaviour
{
    ...
    void Start()
    {
        if (!PlayerPrefs.HasKey("HalcyonEasy High Score"))
        {
            PlayerPrefs.SetInt("HalcyonEasy High Score", 0);
        }
        Application.targetFrameRate = 300;
        secPerBeat = 60f / bpm;
        String path = Application.dataPath + "/StreamingAssets";
        MidiFile midiFile = MidiFile.Read(path + "/haleasy.mid");
    ...
    void Update()
    {
        ...
        if (songposition >= 157)
        {
            PlayerPrefs.SetInt("HalcyonEasy Current Score", GameManager.instance.score);
            PlayerPrefs.SetInt("HalcyonEasy Max Combo", GameManager.instance.maxCombo);
            PlayerPrefs.SetInt("HalcyonEasy Notes Hit", GameManager.instance.notesHit);
            if (PlayerPrefs.GetInt("HalcyonEasy High Score") < GameManager.instance.score)
            {
                PlayerPrefs.SetInt("HalcyonEasy HighScore", GameManager.instance.score);
            }
            SceneManager.LoadScene("HalcyonEasy ScoreScene");
        }
    }
}
