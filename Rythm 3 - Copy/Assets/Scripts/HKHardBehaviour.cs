...
public class HKHardBehaviour : MonoBehaviour
{
    ...
    void Start()
    {
        if (!PlayerPrefs.HasKey("HK High Score"))
        {
            PlayerPrefs.SetInt("HK High Score", 0);
        }
        Application.targetFrameRate = 300;
        secPerBeat = 60f / bpm;
        String path = Application.dataPath + "/StreamingAssets";
        MidiFile midiFile = MidiFile.Read(path + "/hk2.mid");
    ...
    void Update()
    {
        ...
        if (songposition >= 133)
        {
            PlayerPrefs.SetInt("HK Current Score", GameManager.instance.score);
            PlayerPrefs.SetInt("HK Max Combo", GameManager.instance.maxCombo);
            PlayerPrefs.SetInt("HK Notes Hit", GameManager.instance.notesHit);
            if (PlayerPrefs.GetInt("HK High Score") < GameManager.instance.score)
            {
                PlayerPrefs.SetInt("HK HighScore", GameManager.instance.score);
            }
            SceneManager.LoadScene("HKHard ScoreScene");
        }
    }
}
