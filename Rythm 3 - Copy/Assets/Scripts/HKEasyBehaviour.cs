...
public class HKEasyBehaviour : MonoBehaviour
{
    ...
    void Start()
    {
        if (!PlayerPrefs.HasKey("HKE High Score"))
        {
            PlayerPrefs.SetInt("HKE High Score", 0);
        }
        Application.targetFrameRate = 300;
        secPerBeat = 60f / bpm;
        String path = Application.dataPath + "/StreamingAssets";
        MidiFile midiFile = MidiFile.Read(path + "/hkeasy.mid");
    ...
    void Update()
    {
        ...
        if (songposition >= 133)
        {
            PlayerPrefs.SetInt("HKE Current Score", GameManager.instance.score);
            PlayerPrefs.SetInt("HKE Max Combo", GameManager.instance.maxCombo);
            PlayerPrefs.SetInt("HKE Notes Hit", GameManager.instance.notesHit);
            if (PlayerPrefs.GetInt("HKE High Score") < GameManager.instance.score)
            {
                PlayerPrefs.SetInt("HKE HighScore", GameManager.instance.score);
            }
            SceneManager.LoadScene("HKEasy ScoreScene");
        }
    }
}
